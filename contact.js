document.addEventListener("DOMContentLoaded", function () {
  fetch(
    "https://apisitem.ecommcube.com/api/public/industry_template_data/65f0aa0e87406689f2ece77c/661e433e5ef9ee4c88efc8b7/data/contact_us"
  )
...
function displayData(content) {
  const container = document.getElementById("data-container-contact");
  const heading = document.createElement("h1");
  const paragraph1 = document.createElement("p");
  const paragraph2 = document.createElement("p");
  const paragraph3 = document.createElement("p");
  heading.textContent = "Contact Us";
  paragraph1.textContent = content?.value?.message;
  paragraph2.innerHTML = `<strong>Email:</strong> ${content?.value?.email}`;
  paragraph3.innerHTML = `<strong>Phone:</strong> ${content?.value?.phone}`;
  container.innerHTML = "";
